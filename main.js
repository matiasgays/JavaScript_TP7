...
const guardarCompras = (clave, valor) => {localStorage.setItem(clave, valor)};
...
const dom = () => {
    let desplegables = document.getElementsByClassName('desplegables')[0];
    let h3 = document.createElement('h3')
    h3.innerText = `Carrito de compras`;
    desplegables.appendChild(h3);
}
...
while((cantidad <= 2) && (salir == 0)){
    ...
    switch(opcion){
        case '1':
            guardarCompras(`${cantidad}`,JSON.stringify(comprar()));
            cantidad ++;
            break;
